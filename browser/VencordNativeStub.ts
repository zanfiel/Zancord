...
window.VencordNative = {
    ...
    updater: {
        getRepo: async () => ({ ok: true, value: "https://github.com/Zancord/Zancord" }),
    ...
    settings: {
        get: () => {
            try {
                return JSON.parse(localStorage.getItem("ZancordSettings") || "{}");
            } catch (e) {
                console.error("Failed to parse settings from localStorage: ", e);
                return {};
            }
        },
        set: async (s: Settings) => localStorage.setItem("ZancordSettings", JSON.stringify(s)),
