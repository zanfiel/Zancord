...
import { ZANCORD_HELPERS, ZancordDevsById, GUILD_ID, SUPPORT_CHANNEL_ID, VencordDevsById } from "./constants";
...
export const isZancordPluginDev = (id: string) => Object.hasOwn(ZancordDevsById, id);
export const shouldShowZancordContributorBadge = (id: string) => isZancordPluginDev(id) && ZancordDevsById[id].badge !== false;

export const isAnyPluginDev = (id: string) => Object.hasOwn(VencordDevsById, id) || Object.hasOwn(ZancordDevsById, id);
...
export function isZancordGuild(id: string | null | undefined, isGuildId: boolean = false): boolean {
    if (!id) return false;
...
export function isZancordSupport(userId: string | null | undefined): boolean {
    if (!userId) return false;

    const member = GuildMemberStore.getMember(GUILD_ID, userId);
    if (!member) return false;
    return member.roles.includes(ZANCORD_HELPERS) || false;
}
