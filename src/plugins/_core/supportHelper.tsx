...
import { platformName } from "@zancordplugins/zancordHelper/utils";
import customIdle from "@plugins/customIdle";
import { gitHash, gitHashShort } from "@shared/vencordUserAgent";
import { CONTRIB_ROLE_ID, Devs, DONOR_ROLE_ID, ZANCORD_TEAM, GUILD_ID, SUPPORT_CHANNEL_ID, SUPPORT_CHANNEL_IDS, VC_CONTRIB_ROLE_ID, VC_DONOR_ROLE_ID, VC_GUILD_ID, VC_REGULAR_ROLE_ID, VENCORD_CONTRIB_ROLE_ID } from "@utils/constants";
import { sendMessage } from "@utils/discord";
import { Logger } from "@utils/Logger";
import { Margins } from "@utils/margins";
import { isAnyPluginDev, isZancordGuild, isZancordSupport, isSupportChannel, tryOrElse } from "@utils/misc";
...
const TrustedRolesIds = [
    ...
    ZANCORD_TEAM, // Zancord Team
    DONOR_ROLE_ID, // Zancord Donor
    CONTRIB_ROLE_ID, // Zancord Contributor
...
export default definePlugin({
    ...
    commands: [
        {
            name: "Zancord-debug",
            description: "Send Zancord debug info",
            // @ts-ignore
            predicate: ctx => isAnyPluginDev(UserStore.getCurrentUser()?.id) || isZancordGuild(ctx?.guild?.id, true),
            execute: async () => ({ content: await generateDebugInfoMessage() })
        },
        {
            name: "Zancord-plugins",
            description: "Send Zancord plugin list",
            // @ts-ignore
            predicate: ctx => isAnyPluginDev(UserStore.getCurrentUser()?.id) || isZancordGuild(ctx?.guild?.id, true),
            execute: async () => {
        ...
        async CHANNEL_SELECT({ channelId }) {
            ...
            if (!IS_UPDATER_DISABLED) {
                ...
                if (isOutdated) {
                    return Alerts.show({
                        title: "Hold on!",
                        body: <div>
                            <Paragraph>You are using an outdated version of Zancord! Chances are, your issue is already fixed.</Paragraph>
            ...
            if (!IS_STANDALONE && !settings.store.dismissedDevBuildWarning) {
                return Alerts.show({
                    title: "Hold on!",
                    body: <div>
                        <Paragraph>You are using a custom build of Zancord, which we do not provide support for!</Paragraph>
    ...
    renderMessageAccessory(props) {
        ...
        const ZancordSupport = isZancordSupport(props.message.author.id);

        const shouldAddUpdateButton =
            !IS_UPDATER_DISABLED
            && ((isSupportChannel(props.channel.id) && ZancordSupport))
        ...
        if (isSupportChannel(props.channel.id) && PermissionStore.can(PermissionsBits.SEND_MESSAGES, props.channel) && ZancordSupport) {
            if (props.message.content.includes("/Zancord-debug") || props.message.content.includes("/Zancord-plugins")) {
                buttons.push(
                    <Button
                        key="vc-dbg"
    ...
    renderContributorDmWarningCard: ErrorBoundary.wrap(({ channel }) => {
        ...
        return (
            <Card variant="warning" className={Margins.top8} defaultPadding>
                Please do not private message Zancord & Vencord plugin developers for support!
                <br />
