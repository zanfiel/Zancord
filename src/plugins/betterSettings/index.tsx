...
import { buildPluginMenuEntries, buildThemeMenuEntries } from "@zancordplugins/zancordToolbox/menu";
...
export default definePlugin({
        ...
        { // Settings cog context menu
            ...
            replacement: [
                ...
                },
                {
                    ...
                    replace: "$&case 'zancordplugins':return $self.buildPluginMenuEntries(true);case 'ZancordThemes':return $self.buildThemeMenuEntries();"
                }
            ]
        },
    ],
    ...
    transformSettingsEntries(list: SettingsEntry[], keyMap: Map<string, string>) {
        const items: any[] = [];
        const ZancordItems: SettingsEntry[] = [
            { section: "ZancordSettings", label: "Settings" },
            { section: "zancordplugins", label: "Plugins" },
            { section: "ZancordThemes", label: "Themes" },
            { section: "ZancordUpdater", label: "Updater" },
            { section: "ZancordChangelog", label: "Changelog" },
            { section: "ZancordCloud", label: "Cloud" },
            { section: "ZancordBackupAndRestore", label: "Backup & Restore" },
            { section: "ZancordPatchHelper", label: "Patch Helper" },
        ];
        ...
        for (const [section, key] of SettingsPlugin.settingsSectionMap) {
            ...
            if (entry) ZancordItems.push({ section, label: entry.title });
        }
        ...
        if (settings.store.organizeMenu) {
            ...
            keyMap.set("Zancord", "Zancord");
            items.push({
                section: "Zancord",
                items: ZancordItems
            });
        } else {
            items.push(...list, ...ZancordItems);
        }
