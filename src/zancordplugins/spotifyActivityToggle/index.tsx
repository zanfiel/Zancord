...
import zancordToolbox from "@zancordplugins/zancordToolbox";
...
function SpotifyActivityToggleButton({ iconForeground, hideTooltips, nameplate }: UserAreaRenderProps) {
    ...
    if (!isLoaded || !spotifyId || location !== "PANEL" && isPluginEnabled(zancordToolbox.name)) return null;
...
const settings = definePluginSettings({
    location: {
        ...
        options: [
            ...
            { label: "Zancord Toolbox", value: "TOOLBOX" }
        ],
        get hidden() {
            return !isPluginEnabled(zancordToolbox.name);
        }
    },
});
