...
import { ChromiumIcon, ClientIcon, DevBannerIcon, DiscordIcon, ElectronIcon, ZancordIcon, names, settings } from ".";
...
export function makeDevBanner(state?: string): string | JSX.Element {
    ...
    const format = settings.store.format ?? "{devbannerIcon} {buildChannel} {buildNumber} ({buildHash}) | {ZancordIcon} {ZancordName} {ZancordVersion} ({ZancordHash})";
    ...
    const replaced = baseFormat
        ...
        .replace(/{ZancordVersion}/g, VERSION)
        .replace(/{ZancordHash}/g, gitHashShort)
        .replace(/{ZancordPlatform}/g, getVersionInfo(false))
    ...
    const parts = replaced.split(/({.*?}|__NEWLINE__)/).filter(Boolean).map((part, i) => {
        switch (part) {
            ...
            case "{ZancordIcon}":
                return <span key={`icon-Zancord-${i}`} className="vc-discord-dev-banner-icons"><ZancordIcon /></span>;
            case "{electronIcon}":
