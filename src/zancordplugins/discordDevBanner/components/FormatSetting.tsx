...
export function FormatSetting(setValue) {
    ...
    const [state, setState] = useState(format ?? "{buildChannel} {buildNumber} ({buildHash}) | {ZancordName} {ZancordVersion} ({ZancordHash})");
    ...
    return (
        <section>
            ...
            <textarea
                ...
                placeholder="{buildChannel} {buildNumber} ({buildHash}) | {ZancordName} {ZancordVersion} ({ZancordHash})"
                rows={1}
                ref={el => {
                    if (el) {
