...
if (!IS_VANILLA && !IS_EXTENSION) {
    app.whenReady().then(() => {
        ...
        protocol.handle("Zancord", ({ url: unsafeUrl }) => {
            let url = decodeURI(unsafeUrl).slice("Zancord://".length).replace(/\?v=\d+$/, "");
        ...
        try {
            if (RendererSettings.store.enableReactDevtools)
                installExt("fmkadmapgofadopljbjfkapdkoienihi")
                    .then(() => console.info("[Zancord] Installed React Developer Tools"))
                    .catch(err => console.error("[Zancord] Failed to install React Developer Tools", err));
        } catch { }
