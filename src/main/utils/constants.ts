...
export const DATA_DIR = process.env.ZANCORD_USER_DATA_DIR ?? (
    process.env.DISCORD_USER_DATA_DIR
        ? join(process.env.DISCORD_USER_DATA_DIR, "..", "ZancordData", suffix)
        : join(app.getPath("userData"), "..", "Zancord", suffix)
);
    ...
    if (!migrated) {
        setTimeout(() => {
            try {
                ...
                console.error("[Zancord] Failed to copy prod data:", err);
            }
        }, 5000);
    }
}
