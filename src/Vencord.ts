    ...
    if (!hasCloudAuth) {
        if (Settings.cloud.authenticated) {
            ...
            showNotification({
                ...
                onClick: () => SettingsRouter.openUserSettings("Zancord_cloud_panel")
            });
...
async function runUpdateCheck() {
    ...
    try {
        ...
        showNotice(
            "A new version of Zancord is available!",
            "View Update",
...
function initTrayIpc() {
    ...
    VencordNative.tray.onCheckUpdates(async () => {
        try {
            ...
            if (isOutdated) {
                showNotice("A Zancord update is available!", "View Update", () => openSettingsTabModal(UpdaterTab!));
            } else {
    ...
    VencordNative.tray.onRepair(async () => {
        try {
            ...
            UpdateLogger.error("Failed to repair Zancord", err);
        }
    });
    ...
    if (IS_DEV) {
        ...
        if (pendingPatches.length)
            PMLogger.warn(
                ...
                "You are seeing this warning because this is a Development build of Zancord.",
