...
async function buildExtension(target, files) {
    const entries = {
        "dist/Zancord.js": await readFile("dist/browser/extension.js"),
        "dist/Zancord.css": await readFile("dist/browser/extension.css"),
...
const appendCssRuntime = readFile("dist/Zancord.user.css", "utf-8").then(content => {
    const cssRuntime = `unsafeWindow._vcUserScriptRendererCss=\`${content.replaceAll("`", "\\`")}\``;

    return appendFile("dist/Zancord.user.js", cssRuntime);
});
