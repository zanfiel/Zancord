...
While it was primarily created for Zancord based on Vencord and its respective packages, other client mods could also benefit from this, so it is published as a standalone package!

## Installation

```bash
npm install -D @zancord/discord-types
yarn add -D @zancord/discord-types
pnpm add -D @zancord/discord-types
```

## Example Usage

```ts
import type { UserStore } from "@zancord/discord-types";
...
```ts
import { ApplicationCommandType } from "@zancord/discord-types/enums";
